...
const AdminScholarshipList = () => {
  ...
  const fetchScholarships = useCallback(async (params = {}) => {
    ...
    setLoading(true);
    try {
      const { data, error } = await apiService.getAdminScholarships(params);
  ...
  // Filter scholarships based on search term, country and degree
  useEffect(() => {
    const params = {};
    if (searchTerm) params.search = searchTerm;
    if (countryFilter) params.country = countryFilter;
    if (degreeFilter) params.degree_level = degreeFilter;
    fetchScholarships(params);
  }, [searchTerm, countryFilter, degreeFilter, fetchScholarships]);
  ...
  const handleDelete = async (id, scholarshipName) => {
    ...
    try {
      console.log("🗑️ Attempting to delete scholarship with ID:", id);
      ...
      console.log("✅ Scholarship deleted successfully:", data);
      alert("Scholarship deleted successfully!");
    } catch (error) {
      console.error("🚨 Error deleting scholarship:", error);
      alert(`Failed to delete scholarship: ${error.message}`);
    }
  };
          ...
          <div className="px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            ...
            <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
              ...
              <select
                ...
                className="w-full md:w-48 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600"
              >
